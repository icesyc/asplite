...
Public Property Get maxSize			'最大上传尺寸
	maxSize = maxSize_
End Property
Public Property Let maxSize(Value)	'最大上传尺寸
	maxSize_ = Value
End Property
...
Private Sub Class_Initialize
	...
	NoAllowExt="asp;php;jsp;exe;htm;html;aspx;cs;vb;js;"
...
Private Function SaveToFileEx(Item,Path,Over)
	...
	if file.Exists(Item) then
		...
		Else
			...
			if Err.number<>0 then OutErr("自动保存文件出错,已经测试50次不同的文件名来保存,请检查目录是否存在!该文件最后一		次保存时全路径为"&Path&GetNewFileName()&"."&File(Item).FileExt)
			Else
				isErr_=3
				ErrMessage_="该后缀名的文件不允许上传!"
				OutErr("该后缀名的文件不允许上传")
